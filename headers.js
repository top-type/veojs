  ...
  if(ls_check){
   ...
   var top_diff = 0;//accumulative difficulty of top
    ...
    function write_header(header, ewah) {
        ...
        if ((acc_difficulty > top_diff) || (header[1] > top_header[1])) {
	...
	*/
    ...
    function header_test2(hl) {
		...
		var onchange = function () {console.log(top_header[1]);};
		var changeHeight = top_header[1];
		...
		setInterval(on_height_change, 1000);
    ...
    function test() {
        ...
        return {more_headers: more_headers,
                ...
                db: headers_db,
                read_ewah: read_ewah,
                onchange: function(func) {onchange = func;},
                forks: forks};
}
